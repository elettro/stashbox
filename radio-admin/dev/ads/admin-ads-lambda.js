...
function matchesAdminVecSongAssetsRoute(event, route) {
  return routeEndsWith(route, '/admin/vec/song-assets') || routeEndsWith(event.rawPath, '/admin/vec/song-assets') || routeEndsWith(event.path, '/admin/vec/song-assets') || routeEndsWith(event.routeKey, '/admin/vec/song-assets') || routeEndsWith(event.resource, '/admin/vec/song-assets') || getRouteSegments(event).slice(0, 3).join('/') === 'admin/vec/song-assets';
}
...
async function getTableColumns(schemaName, tableName) {
  const metadata = await getTableColumnMetadata(schemaName, tableName);
  return new Set(metadata.keys());
}

async function getTableColumnMetadata(schemaName, tableName) {
  const result = await client.query(
...
async function buildSafeSongInsert(input, columnMetadata) {
  ...
  const normalizedPayload = Object.fromEntries(insertEntries);
  await validateSongForeignKeyValues(normalizedPayload, foreignKeys);
...
async function buildSafeSongUpdate(input, columnMetadata) {
  const foreignKeys = await getSongForeignKeyMetadata();
  const updateEntries = Object.entries(buildSongPayload(input, { partial: true }))
    .filter(([field, value]) => value !== undefined && columnMetadata.has(field) && field !== 'song_key')
    .map(([field, value]) => [field, normalizeSongLookupInsertValue(field, value, foreignKeys)])
    .map(([field, value]) => [field, normalizeSongInsertValue(field, value, columnMetadata.get(field))])
    .filter(([, value]) => value !== undefined);

  const normalizedPayload = Object.fromEntries(updateEntries);
  await validateSongForeignKeyValues(normalizedPayload, foreignKeys);
...
async function canAutoCreateAlbum() {
  const columns = await getTableColumnMetadata('radio', 'albums');
...
function isDevRuntime() {
  ...
  ].some((value) => String(value || '').toLowerCase().includes('dev'));
}
...
async function createAdminSong(event) {
  ...
  try {
    ...
    ({ payload, fields, values } = await buildSafeSongInsert(body, columnMetadata));
...
async function updateAdminSong(event) {
  ...
  try {
    await ensureSongExperienceColumns();
    columnMetadata = await getTableColumnMetadata('radio', 'songs');
    ({ payload, fields, values } = await buildSafeSongUpdate(body, columnMetadata));
    if (!fields.length) return response(400, { success: false, error: 'No editable fields provided.' });

    values.push(songKey);
    const updatedAt = columnMetadata.has('updated_at') ? ', updated_at = now()' : '';
    const result = await client.query(
      `UPDATE radio.songs
...
async function trackSongEvent(client, event) {
  ...
  if (normalizedEventType === 'share') {
    ...
    console.log('[Stashbox Radio API Dev] share update result', {
      rowCount: result.rowCount,
      rows: result.rows
    });

    if (!result.rowCount) {
      console.warn('[Stashbox Radio API Dev] share did not match song', {
        songKey,
        songId
      });
      return response(404, {
...
async function getSongVisualAssets(event) {
  const validation = validateVecSongKey(event.queryStringParameters?.song_key || event.queryStringParameters?.songKey);
  if (validation.error) return response(400, { success: false, error: validation.error });
  await ensureSongVisualAssetsTable();
  const result = await client.query(`SELECT * FROM radio.song_visual_assets WHERE song_key = $1 AND status <> 'hidden' ORDER BY created_at ASC, file_name ASC NULLS LAST`, [validation.songKey]);
  const assets = result.rows.map(normalizeSongVisualAsset);
  return response(200, { success: true, song_key: validation.songKey, images: assets.filter((asset) => asset.asset_type === 'image'), clips: assets.filter((asset) => asset.asset_type === 'clip'), assets });
}
