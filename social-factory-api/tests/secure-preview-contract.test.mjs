...
test('browser response includes internal signed URL and stable absolute URL',async()=>{const x=make(),r=await x.s.preview(event({origin:'https://stashbox.com'}),id);assert.equal(r.expires_in_seconds,900);assert.ok(r.preview_url);assert.equal(r.review_page_url,`https://stashbox.com/radio-admin/dev/social-factory/content-review/preview/?review_id=${id}`);assert.equal(x.writes(),0)});
