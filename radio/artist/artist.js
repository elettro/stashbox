(() => {
  ...
  function startSong(song, mode = 'single') {
    ...
    location.href = '/radio/?artist_radio=1';
  }
  ...
  function bind() {
    app.querySelector('[data-back]')?.addEventListener('click', () => location.href = '/radio/');
  ...
  async function shareProfile() {
    const url = new URL('/radio/artist/', location.origin);
  ...
  async function init() {
    try {
      ...
      app.innerHTML = `<section class="artist-load-error"><strong>STASH<span>BOX</span></strong><h1>Artist profile could not load</h1><p>${esc(error.message || 'Unknown error')}</p><a href="/radio/">Return to Radio</a></section>`;
    }
  }
