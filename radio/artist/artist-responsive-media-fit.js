(() => {
  ...
  if (!window.location.pathname.includes('/radio/artist/')) return;
  ...
  function fitMedia(media) {
    if (!media) return;
    const desktop = desktopSurface();
    const active = media.classList.contains('is-active');

    media.style.removeProperty('visibility');
    media.style.removeProperty('pointer-events');
    media.style.objectFit = desktop ? 'contain' : 'cover';
    media.style.objectPosition = 'center center';
    media.dataset.artistDesktopRatioState = desktop ? 'approved-contained' : 'mobile-approved';

    if (active) media.style.setProperty('opacity', '1', 'important');
    else media.style.removeProperty('opacity');
  }
  ...
  function bindMedia(media) {
    ...
    ['load', 'loadedmetadata', 'loadeddata', 'canplay', 'playing'].forEach(eventName => {
      media.addEventListener(eventName, () => inspectMedia(media), { passive: true });
    });
    inspectMedia(media);
  }
  ...
  function scheduleApply(delay = 0, options = {}) {
    ...
    scheduled = window.setTimeout(() => {
      applyCurrent(options).catch(error => {
        console.warn('[Artist responsive media] Artwork application failed.', error?.message || error);
      });
    }, delay);
  }
  ...
  function warmFromTarget(target) {
    ...
    catalog().then(songs => {
      ...
      if (!song) return;
      artworkForSong(key).then(images => preload(chooseArtwork(song, images).url)).catch(() => {});
    }).catch(() => {});
  }
