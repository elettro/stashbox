(() => {
  ...
  function bottomNavMarkup() {
    return `<nav class="profile-bottom-nav" aria-label="Primary navigation">
      <a href="/radio/">${icons.home}<span>Home</span></a>
      <a href="/radio/#popular-artists">${icons.compass}<span>Explore</span></a>
      <a href="/radio/">${icons.play}<span>Player</span></a>
      <a href="https://stashbox.ai" target="_blank" rel="noopener">${icons.shop}<span>Shop</span></a>
      <a class="active" href="/radio/profile/">${icons.user}<span>Profile</span></a>
    </nav>`;
  }

  function renderProfile() {
    state.view = 'profile';
    history.replaceState(null, '', '/radio/profile/');
  ...
  function renderSettings() {
    state.view = 'settings';
    history.replaceState(null, '', '/radio/profile/?view=settings');
    const profile = settings();
  ...
  async function loadAll() {
    if (!state.tokens?.accessToken) {
      location.replace('/radio/?auth=login&return=profile');
      return;
    }
    try {
      ...
      app.innerHTML = `<section class="profile-error"><strong>STASH<span>BOX</span></strong><h1>Profile could not load</h1><p>${escapeHtml(error.message || 'Unknown error')}</p><a href="/radio/">Return to Radio</a></section>`;
    }
  }
  ...
  document.addEventListener('click', async event => {
    ...
    if (target.closest('[data-open-notifications]')) {
      ...
      location.href = '/radio/?open_notifications=1';
      return;
    }
    ...
    if (artistKey) return location.href = `/radio/artist/?artist=${encodeURIComponent(artistKey)}`;
