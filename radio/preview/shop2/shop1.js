(function () {
  ...
  function renderMobileShop(productRow, products) { const mount = document.getElementById(MOBILE_MOUNT); if (!mount) return; mount.innerHTML = `<div class="mobile-shop-drawer"><button class="mobile-shop-toggle" type="button"><span>SHOP</span><strong>${products.length} items</strong></button><div class="mobile-shop-panel"><div class="mobile-shop-kicker">STASHBOX MERCH</div><h3>${escapeHtml(productRow.merchHeadline || 'Shop This Track')}</h3><div class="mobile-shop-list">${products.map(p => renderProductCardHtml(p, productRow)).join('')}</div></div></div>`; const btn = mount.querySelector('.mobile-shop-toggle'); const panel = mount.querySelector('.mobile-shop-panel'); if (btn && panel) btn.addEventListener('click', () => { const open = panel.style.display !== 'none'; panel.style.display = open ? 'none' : 'block'; }); }
