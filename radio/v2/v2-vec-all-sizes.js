(() => {
  ...
  function unwrap(data) {
    if (typeof data?.body === 'string') {
      try {
        return unwrap(JSON.parse(data.body));
      } catch (_) {
        return data;
      }
    }
  ...
  function currentSongFromPlayer(player) {
    ...
    return state.songs.find(song => (
      ...
      (!artist || normalize(song.artist) === artist)
    )) || state.songs.find(song => normalize(song.title) === title) || null;
  }
  ...
  function assetType(asset) {
    ...
    return value === 'clip' || value === 'video' || value.startsWith('video/')
      ? 'clip'
      : 'image';
  }
  ...
  function normalizeAsset(asset, source, folder = null) {
    ...
    return {
      ...
      durationSeconds: Math.max(
        1,
        Number(asset.duration_seconds || asset.durationSeconds || 0) || 0
      ),
      alt: clean(
        asset.alt_text ||
        asset.altText ||
  ...
  function enabledFolders(recipe) {
    return (Array.isArray(recipe?.folders) ? recipe.folders : [])
  ...
  async function fetchSongAssets(songKey, source = 'song') {
    ...
    return rows(body, ['assets', 'items', 'data'])
      .map(asset => normalizeAsset(asset, source))
      .filter(Boolean);
  }
  ...
  async function fetchFolderAssets(folder) {
    ...
    const assets = rows(body, ['assets', 'items', 'data'])
      .map(asset => normalizeAsset(asset, `folder:${folderId}`, folder))
      .filter(Boolean);
  ...
  async function loadVec(song, run) {
    ...
    const folderGroups = await Promise.all(
      folderEntries.map(folder => fetchFolderAssets(folder).catch(() => []))
    );
    if (run !== state.currentRun) return;

    const borrowedGroups = await Promise.all(
      borrowedSources(recipe)
        .filter(source => source?.enabled !== false)
        .map(async source => {
          const sourceKey = clean(
            source.song_key ||
            source.source_song_key ||
            source.key ||
            source.id
          );
          if (!sourceKey) return [];
          const assets = await fetchSongAssets(sourceKey, `borrowed:${sourceKey}`)
            .catch(() => []);
    ...
    const artwork = {
      ...
      durationSeconds: Math.max(
        2,
        Number(
          recipe?.artwork?.start_duration_seconds ||
          recipe?.artwork_rules?.start_duration_seconds ||
          4
        ) || 4
      ),
    ...
    const manual = Array.isArray(recipe?.manual_sequence)
      ? recipe.manual_sequence
      : Array.isArray(recipe?.sequence)
        ? recipe.sequence
        : [];
    const orderMode = clean(
      recipe?.shuffle?.order_mode ||
      recipe?.shuffle_rules?.order_mode ||
      recipe?.order_mode
    ).toLowerCase();
    ...
    if (orderMode === 'manual' && manual.length) {
      ...
      sequence = manual.map(entry => {
        ...
        return {
          ...
          durationSeconds: Math.max(
            1,
            Number(entry.duration_seconds || entry.durationSeconds || asset.durationSeconds || 4)
          )
        };
      }).filter(Boolean);
    } else {
      ...
      const startWithArtwork = (
        recipe?.artwork?.start_with_artwork !== false &&
        recipe?.artwork_rules?.start_with_artwork !== false
      );
      sequence = startWithArtwork ? [artwork, ...bag] : bag;
    }

    if (!sequence.length) sequence = [artwork];
    const label = sequence.some(asset => asset.type === 'clip')
      ? 'VEC Visuals'
      : 'VEC Images';
    startSequence(song, recipe, sequence, run, label);
  }
  ...
  function stopVec({ remove = false } = {}) {
    ...
    player?.classList.remove('is-mobile-vec-active', 'is-vec-active');
  ...
  function playerAudio(player) {
    return player?.querySelector('[data-audio]') || null;
  }

  function pauseActiveVisual() {
    clearTimeout(state.resumeTimer);
    state.resumeTimer = 0;
    if (state.activeMedia?.tagName === 'VIDEO' && !state.activeMedia.paused) {
      try {
        state.activeMedia.pause();
      } catch (_) {}
    }
  ...
  function resumeActiveVisual(player) {
    const audio = playerAudio(player);
    const video = state.activeMedia?.tagName === 'VIDEO' ? state.activeMedia : null;
    if (!video || video.ended || !video.isConnected || !audio || audio.paused || audio.ended) return;
    video.muted = true;
  ...
  function bindAudioSync(player) {
    const audio = playerAudio(player);
    if (!audio || audio.dataset.vecAllSizesSyncBound === 'true') return;
    audio.dataset.vecAllSizesSyncBound = 'true';
    audio.addEventListener('pause', pauseActiveVisual);
    audio.addEventListener('ended', pauseActiveVisual);
    audio.addEventListener('emptied', pauseActiveVisual);
    audio.addEventListener('play', () => {
      clearTimeout(state.resumeTimer);
      state.resumeTimer = window.setTimeout(() => resumeActiveVisual(player), 150);
    });
  }
  ...
  async function togglePrimaryMedia(player) {
    ...
    try {
      await audio.play();
      clearTimeout(state.resumeTimer);
      state.resumeTimer = window.setTimeout(() => resumeActiveVisual(player), 180);
    } catch (error) {
      pauseActiveVisual();
      console.warn('[V2 VEC] Song resume failed', error?.message || error);
    }
  }
  ...
  function renderAsset(song, recipe, run) {
    ...
    window.dispatchEvent(new CustomEvent('stashbox:vec-asset-change', {
      detail: { songKey: song.key, asset }
    }));

    if (asset.type === 'clip' && asset.productUrls.length) {
      showClipCommerce(song, asset);
    }
    ...
    if (asset.type === 'clip') {
      ...
      const audio = playerAudio(player);
      if (!audio || !audio.paused) media.play().catch(() => {});
      ...
      const duration = Math.max(
        ...
        Math.min(
          ...
          (
            ...
            recipe?.render?.still_image_duration_seconds ||
            recipe?.render_settings?.still_image_duration_seconds ||
            6
          ) * 1000
        )
      );
  ...
  async function loadProducts() {
    ...
    if (!state.productsPromise) {
      state.productsPromise = json(SHOP_URL)
        .then(body => rows(body, ['products']).map(product => ({
  ...
  async function showClipCommerce(song, asset) {
    ...
    tray.innerHTML = `
      <header class="v2-vec-clip-commerce-head">
        <strong>${bagIcon}<span>Shop the Clip</span></strong>
        <button type="button" data-vec-commerce-close>
          Closes in <b data-vec-commerce-seconds>${seconds}</b>s
        </button>
      </header>
      <div class="v2-vec-clip-commerce-row">
        ${products.map(product => `
          <a class="v2-vec-clip-product"
             href="${esc(product.url)}"
             target="_blank"
             rel="noopener"
             data-vec-product-click="${esc(product.url)}">
            <span>
              ${product.image
    ...
    state.commerceTimer = window.setTimeout(
      () => hideClipCommerce(),
      CLIP_PRODUCT_LIFETIME_MS
    );
  }
  ...
  async function tick() {
    const player = currentPlayer();
    if (!loggedIn() || !playerIsOpen(player)) {
    ...
    loadVec(song, run).catch(error => {
      console.warn('[V2 VEC]', error.message || error);
      if (run !== state.currentRun) return;
      startSequence(song, {}, [{
        id: `artwork:${song.key}`,
        key: `artwork:${song.key}`,
  ...
  document.addEventListener('click', event => {
    ...
    const playButton = event.target.closest(
      '#v2App .v2-player.is-logged-in-player [data-play]'
    );
    if (playButton && playerIsOpen(player) && loggedIn()) {
    ...
    if (product) {
      trackProductClick(product.dataset.vecProductClick || product.href);
    }
  }, true);
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    if (document.hidden) {
      pauseActiveVisual();
    } else if (playerIsOpen(player)) {
      resumeActiveVisual(player);
    }
  });
