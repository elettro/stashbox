(() => {
  ...
  function bind() {
    app.querySelector('[data-back]')?.addEventListener('click', () => location.href = '/radio/attempt2/');
  ...
  async function shareProfile() {
    const url = new URL('/radio/attempt2/artist/', location.origin);
  ...
  async function init() {
    try {
      ...
      app.innerHTML = `<section class="artist-load-error"><strong>STASH<span>BOX</span></strong><h1>Artist profile could not load</h1><p>${esc(error.message || 'Unknown error')}</p><a href="/radio/attempt2/">Return to Radio</a></section>`;
    }
  }
