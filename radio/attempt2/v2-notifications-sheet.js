(() => {
  ...
  function openNotification(item) {
    ...
    if (songKey) {
      ...
      const target = new URL('/radio/attempt2/', window.location.origin);
    ...
    if (item.action_url) {
      try {
        const url = new URL(item.action_url, window.location.href);
        if (url.origin === window.location.origin && url.pathname.startsWith('/radio/') && !url.pathname.startsWith('/radio/attempt2/')) {
          url.pathname = '/radio/attempt2/';
        }
        closeSheet();
        window.setTimeout(() => {
