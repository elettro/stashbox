(() => {
  ...
  function inject() {
    ...
    if (header && !header.querySelector('.v2-li-player-head-actions')) {
      header.insertAdjacentHTML('beforeend', `<div class="v2-li-player-head-actions"><button type="button" data-li-search>${icon.search}</button><button type="button" data-li-notifications>${icon.bell}</button><a class="v2-li-player-profile" data-li-profile href="/radio/attempt2/profile/">P</a></div>`);
    }
  ...
  function viewArtist() { location.href = `/radio/attempt2/artist/?artist=${encodeURIComponent(state.player.querySelector('[data-li-artist]')?.dataset.artistKey || slug(state.current?.artist))}`; }
  function songPage() {
