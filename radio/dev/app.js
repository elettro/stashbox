...
function normalizeSongVisualImage(asset, song) {
  const publicUrl = fixDropbox(clean(asset?.public_url || asset?.publicUrl || asset?.url || asset?.src));
  if (!publicUrl) return null;
  const songTitle = clean(song?.title || song?.display_title || song?.song_name) || UNTITLED_STASHBOX_TRACK;
  return {
    type: 'image',
    url: publicUrl,
    alt: clean(asset?.alt_text || asset?.altText) || songTitle,
    caption: clean(asset?.caption),
    key: clean(asset?.id || asset?.asset_id || asset?.key || publicUrl),
    source: 'visual-experience'
  };
}

function normalizeSongVisualImagesResponse(data, song) {
  if (typeof data?.body === 'string') {
    try { return normalizeSongVisualImagesResponse(JSON.parse(data.body), song); } catch (_) { return []; }
  }
  return (Array.isArray(data?.images) ? data.images : [])
    .map(asset => normalizeSongVisualImage(asset, song))
    .filter(Boolean);
}
...
function recordDevAdEvent(ad, eventType, { song = null, sessionId = '', extra = {} } = {}) {
  if (!ad?.id) return;
  const event = {
    ad_id: ad.id,
...
function App() {
  ...
  function getEligibleAdsForBreak() {
    ...
    if (!eligible.length) console.log('[Stashbox Radio Dev] No active ads available for completed item');
    return eligible;
  }
  ...
  function buildAdBreakQueue() {
    ...
    if (settings.break_method === 'seconds') {
      ...
      console.log(`[Stashbox Radio Dev] Built seconds ad break: ${queue.length} ads, estimated ${accumulatedSeconds}s target ${settings.target_ad_seconds}s`);
      return queue;
    }
    ...
    console.log(`[Stashbox Radio Dev] Built count ad break: ${queue.length} ads requested ${requestedCount}`);
    return queue;
  }
  ...
  function maybeStartAdBeforeNextSong(nextSong, currentSong, { allowAfterCompletedVideo = false } = {}) {
    ...
    if (!adSettings.ads_enabled) {
      console.log('[Stashbox Radio Dev] Ads disabled by public ad settings.');
      return false;
    }
    ...
    if (adBreakCompletedCountRef.current < breakInterval) {
      console.log(`[Stashbox Radio Dev] Ad break skipped by interval ${adBreakCompletedCountRef.current}/${breakInterval}`);
      return false;
    }
  ...
  function handleAdBlocked(ad) {
    console.warn(`[Stashbox Radio Dev] Ad autoplay blocked or skipped by browser restriction: ${ad?.title || 'Untitled ad'}`);
...
function AdPlayer({ ad, playerRef, adBreakDisplay, adBreakMuted = false, onToggleAdMute, onStarted, onProgress, onCompleted, onSkipped, onBlocked, onCtaClicked, onError, onDurationKnown }) {
  ...
  const mediaMuteLabel = mediaMuted ? 'Unmute audio' : 'Mute audio';

  const applyMediaMutedState = media => {
...
function Player({ selected, audioRef, playerRef, youtubePlayerRef: externalYoutubePlayerRef, mediaMode, activeVideoEmbedUrl, openVideo, closeVideo, products, playerMessage = '', mediaMuted = false, onToggleMediaMute, onPrevious, onNext, onShuffle, onProductClick, likeCount, playCount, shareCount, hasLiked, onLike, onShare, shareCopied, onAudioStart, onAudioProgress, onAudioPause, onAudioComplete, onVideoStart, onVideoProgress, onVideoComplete, onYouTubeEnded, onPlaybackStatusChange, autoPlayRequest, onAdStarted, onAdCompleted, onAdSkipped, onAdCtaClicked, onAdError, onAdDurationKnown }) {
  ...
  const clearVisualRotation = useCallback(() => {
    if (visualRotationTimerRef.current) {
      window.clearTimeout(visualRotationTimerRef.current);
      visualRotationTimerRef.current = null;
    }
  }, []);

  const fetchSongVisuals = useCallback(async (song, { signal } = {}) => {
    ...
    try {
      ...
      let data = null;
      try { data = text ? JSON.parse(text) : null; } catch (_) { data = null; }
      if (!response.ok) throw new Error((data && (data.error || data.message)) || `Visuals API returned HTTP ${response.status}`);
      return normalizeSongVisualImagesResponse(data, song);
    } catch (error) {
      if (error?.name !== 'AbortError') console.warn('[Stashbox Radio Dev] visual images load failed; using song artwork fallback', error?.message || error);
      return [];
    }
  }, []);
  ...
  const startVisualRotation = useCallback((song, images = currentVisualImages) => {
    clearVisualRotation();
    if (!Array.isArray(images) || images.length <= 1 || mediaMode === 'video') return;
    visualRotationTimerRef.current = window.setTimeout(() => {
      setCurrentVisualIndex(index => (index + 1) % images.length);
    }, DEFAULT_VISUAL_STILL_DURATION_SECONDS * 1000);
  }, [clearVisualRotation, currentVisualImages, mediaMode]);
  ...
  useEffect(() => {
    ...
    return clearVisualRotation;
  }, [selected?.idx, selected?.songKey, currentVisualImages, currentVisualIndex, mediaMode, startVisualRotation, clearVisualRotation]);
  useEffect(() => {
  ...
  const showMediaMuteButton = directVideo || (!isVideoMode && hasAudio);
  const mediaMuteLabel = mediaMuted ? 'Unmute audio' : 'Mute audio';
  const applyPlayerMutedState = media => {
    const target = media || (directVideo ? videoFrameRef.current : audioRef.current);
    if (!target) return;
    target.muted = mediaMuted;
    target.volume = mediaMuted ? 0 : 1;
  };
  ...
  }, [mediaMuted, selected?.idx, mediaMode, activeVideoEmbedUrl]);
  ...
  return h('aside', { className: 'panel player', ref: playerRef, tabIndex: -1, 'aria-label': 'Selected song player' },
    h('div', { ref: mediaFullscreenRef, className: `player-media clickable-media${activeVisualAsset && !isVideoMode ? ' enhanced-visual-media' : ''}`, role: 'button', tabIndex: 0, title: 'Play or pause current track', 'aria-label': 'Play or pause current track', onClick: toggleMediaAreaPlayback, onKeyDown: event => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); togglePlayback(); } } }, showMediaMuteButton ? h('button', {
      type: 'button',
