...
function RadioControlBar({ trackCount, isLoading = false, query, onQueryChange, genre, onGenreChange, album, onAlbumChange, artist, onArtistChange, artistFilters = ['ALL'], onResetFilters }) {
  ...
  const resetMobileFilters = useCallback(event => {
    event?.stopPropagation?.();
    onResetFilters?.();
    closeMobileSearch();
  }, [closeMobileSearch, onResetFilters]);
...
function App() {
  ...
  return h('div', { className: 'radio-app' },
    h(RadioControlBar, { trackCount: tracks.length, query, onQueryChange: setQuery, genre, onGenreChange: setGenre, album, onAlbumChange: setAlbum, artist, onArtistChange: setArtist, artistFilters, onResetFilters: resetFilters }),
...
function Player({ selected, audioRef, playerRef, youtubePlayerRef: externalYoutubePlayerRef, mediaMode, activeVideoEmbedUrl, openVideo, closeVideo, products, playerMessage = '', onPrevious, onNext, onShuffle, onProductClick, likeCount, playCount, shareCount, hasLiked, onLike, onShare, shareCopied, onAudioStart, onAudioProgress, onAudioPause, onAudioComplete, onVideoStart, onVideoProgress, onVideoComplete, onYouTubeEnded, onPlaybackStatusChange, autoPlayRequest }) {
  ...
  const stickyIsPlaying = isVideoMode ? isVideoPlaying : isPlaying;
  ...
  return h('aside', { className: 'panel player', ref: playerRef, tabIndex: -1, 'aria-label': 'Selected song player' },
    h('div', { className: 'mobile-sticky-player', 'aria-label': 'Sticky now-playing player' },
      h('button', { className: 'mobile-sticky-art', type: 'button', onClick: togglePlayback, disabled: !canUsePrimaryPlay, 'aria-label': stickyIsPlaying ? `Pause ${selected.title}` : `Play ${selected.title}` },
        posterImage ? h('img', { src: posterImage, alt: `${selected.title} artwork`, onError: e => { e.currentTarget.style.display = 'none'; } }) : h('span', { className: 'mobile-sticky-art-fallback' }, selected.title.slice(0, 2))
      ),
      h('div', { className: 'mobile-sticky-track-info' },
        h('div', { className: 'mobile-sticky-title' }, selected.title),
        h('div', { className: 'mobile-sticky-artist' }, selected.artist || 'Stashbox')
      ),
      h('button', { className: `mobile-sticky-play ${stickyIsPlaying ? 'playing' : ''}`, type: 'button', onClick: togglePlayback, disabled: !canUsePrimaryPlay, 'aria-pressed': stickyIsPlaying, 'aria-label': stickyIsPlaying ? 'Pause song' : 'Play song' }, stickyIsPlaying ? h(PauseIcon) : h(PlayIcon)),
      hasVideo ? h('button', { className: 'mobile-sticky-video', type: 'button', onClick: isVideoMode ? handleCloseVideo : openVideo, 'aria-label': isVideoMode ? 'Close video' : 'Watch video' }, isVideoMode ? 'Close' : 'Watch') : null
    ),
