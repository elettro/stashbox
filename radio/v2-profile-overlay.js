(() => {
  ...
  if (!location.pathname.includes('/radio/') || location.pathname.includes('/profile/') || location.pathname.includes('/artist/')) return;
  if (window.StashboxV2ProfileOverlay) return;

  const TOKEN_KEY = 'stashbox_radio_prod_cognito_tokens';
  const PROFILE_URL = '/radio/profile/?embedded=1';
  let overlay = null;
  ...
  function runProfileQueue() {
    ...
    script.src = `/radio/v2-profile-queue.js?v=20260822-profilequeue-live-${Date.now()}`;
    script.async = true;
