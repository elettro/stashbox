...
Desktop Radio loaded and the logged-in Account state was present, but notifications stopped surfacing from the visible desktop bell.
...
The repair stayed isolated to the existing notification runtimes. No global `fetch` patch and no new MutationObserver were introduced.
...
On 2026-08-21 the user confirmed that notifications are visible again on desktop while logged in. This verifies the primary regression: the visible desktop bell opens the notification surface and the feed populates.

Retained regression checks:

- Radio DEV V2 desktop loads normally.
- Logged-in Account state remains visible after full boot.
- Notifications bell remains visible after full boot.
- Clicking the visible desktop bell opens the notification sheet.
- The logged-in user receives the notification feed.
- Opening notifications does not freeze, spin, or block the player runtime.
...
Closed as Fixed and Verified on 2026-08-21 after user confirmation that desktop notifications are visible again.