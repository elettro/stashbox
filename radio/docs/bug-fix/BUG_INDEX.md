...
| SR-BUG-0024 | Listener profile images load in DEV but fail in production | Listener Profile / Profile Media / PROD Data | High | PROD Desktop + Mobile | Fixed, user verification pending | 2026-08-23 | `54a39ea0`, `e53dfc33`, `e6fcf8ef`, `d0d93ebb`, `242cc306`, `d02c3a43` |
...
- `SR-BUG-0024` - The first repair copied all 3 listener profile-media objects into PROD and verified public reads. The 2026-08-24 retest exposed two remaining production paths: the PROD media bucket had no browser PUT CORS rule, causing `Failed to fetch` during uploads, and the profile-media editor mounted before authenticated media hydration finished, then refused to refresh the already-mounted cards. The same initialization also replaced legacy form media values with empty unified-media state. PROD S3 CORS now allows Stashbox browser uploads. The production profile-media runtime now preserves saved avatar/banner fallbacks, rehydrates mounted cards after the API response arrives, repoints legacy DEV media URLs to the PROD CloudFront host, and is cache-busted as `profilemedia3`. Desktop and mobile verification remain pending.
