...
const SHELL = [
  OFFLINE_URL,
  '/radio/offline/offline.js?v=20260821-offlineaudio1',
  '/radio/offline/manifest.webmanifest?v=20260821-offlineaudio1',
  '/radio/desktop/browser-audio-map.js?v=20260818-stream320-80',
];
...
self.addEventListener('fetch', event => {
  ...
  if (request.mode === 'navigate' && url.origin === self.location.origin && url.pathname.startsWith('/radio/')) {
  ...
  if (url.origin === self.location.origin && (
    url.pathname.startsWith('/radio/offline/') ||
    url.pathname === '/radio/desktop/browser-audio-map.js'
  )) {
    event.respondWith((async () => {
      const cache = await caches.open(CACHE);
