...
function statusMessage() {
  ...
  if (!missing.length) return `Loaded ${state.songs.length} public song row${state.songs.length === 1 ? '' : 's'} for the public dashboard.`;
...
function renderStats() {
  ...
  renderStatGrid(els.operationalStats, [
    ...
    ['Recent Activity', state.summary?.total_events ?? 0],
...
function renderDevices() {
  ...
  state.devices.slice(0, 10).forEach((device, index) => {
    ...
    const activityCount = firstDefined(device, ['event_count', 'events', 'count']);
    ...
    card.querySelector('.rank-value').textContent = `${formatNumber(activityCount)} activity`;
