...
const OUTPUT = process.env.DESKTOP_EDGE_PROBE_OUTPUT || '/tmp/desktop-v2-system-edge.json';
...
try {
  ...
  result = { browser: 'system-edge', ok: false, fatalError: error?.stack || error?.message || String(error), snapshot };
} finally {
  await driver.quit();
}
