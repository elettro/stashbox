...
test('artwork remains latched until an image or video is ready', () => {
  assert.match(main, /v2-media-transition-guard\.css\?v=20260725-transition79/);
  assert.match(main, /v2-media-transition-guard\.js\?v=20260725-transition79/);
...
test('alternate player is a separate route using the same functional engine', () => {
  assert.match(alt, /alt-player-complete-20260725-79/);
  assert.match(alt, /canonical" href="https:\/\/stashbox\.com\/radio\/dev\/v2\/alt-player\//);
  assert.match(alt, /v2-recovery\.js/);
  assert.match(alt, /v2-session-manager\.js/);
  assert.match(alt, /v2-vec-player-controller\.js/);
  assert.match(alt, /v2-mobile-player-swipe\.js/);
  assert.match(alt, /alt-player\.css/);
  assert.match(alt, /alt-player\.js/);
});
