...
const aid=a=>String(a?.id||a?.asset_id||a?.assetId||'').trim();
const aurl=a=>String(a?.public_url||a?.publicUrl||a?.url||a?.asset_url||a?.assetUrl||'').trim();
...
function folderPayload(f){return {
  ...
  relevant_artists:Array.isArray(f.relevant_artists)?f.relevant_artists:(Array.isArray(f.relevantArtists)?f.relevantArtists:[]),relevant_genres:Array.isArray(f.relevant_genres)?f.relevant_genres:(Array.isArray(f.relevantGenres)?f.relevantGenres:[]),relevant_moods:Array.isArray(f.relevant_moods)?f.relevant_moods:(Array.isArray(f.relevantMoods)?f.relevantMoods:[]),relevant_songs:Array.isArray(f.relevant_songs)?f.relevant_songs:(Array.isArray(f.relevantSongs)?f.relevantSongs:[])
}}

function deriveS3Key(asset){
  const explicit=String(asset?.s3_key||asset?.s3Key||asset?.key||asset?.object_key||asset?.objectKey||'').trim();
  if(explicit) return explicit;
  const url=aurl(asset);
  if(!url) return '';
  try{return decodeURIComponent(new URL(url).pathname.replace(/^\/+/,''))}catch{return ''}
}

function folderAssetPayload(asset,folderId){
  const url=aurl(asset);
  return {
    id:aid(asset),
    folder_id:folderId,
    asset_type:asset.asset_type||asset.assetType||asset.type||'video',
    file_name:asset.file_name||asset.fileName||asset.filename||'',
    s3_key:deriveS3Key(asset),
    public_url:url,
    thumbnail_url:asset.thumbnail_url||asset.thumbnailUrl||url,
    content_type:asset.content_type||asset.contentType||'',
    size_bytes:asset.size_bytes??asset.sizeBytes??null,
    width:asset.width??null,
    height:asset.height??null,
    duration_seconds:asset.duration_seconds??asset.durationSeconds??asset.duration??null,
...
function directPayload(songKey,a){return {song_key:songKey,asset_type:a.asset_type||a.assetType||a.type||'image',file_name:a.file_name||a.fileName||a.filename||'',s3_key:deriveS3Key(a),public_url:aurl(a),thumbnail_url:a.thumbnail_url||a.thumbnailUrl||aurl(a),content_type:a.content_type||a.contentType||'',size_bytes:a.size_bytes??a.sizeBytes??null,width:a.width??null,height:a.height??null,ratio_label:a.ratio_label||a.ratioLabel||'',caption:a.caption||'',alt_text:a.alt_text||a.altText||'',notes:a.notes||''}}
...
try{
  ...
  for(const df of devFolders){
    ...
    }catch(e){result.folders.failed.push({dev_folder_id:did,name:df.folder_name||df.folderName||'',error:e.message})}
  }
    ...
    try{
      ...
      for(const d of da){
        ...
        try{
          ...
          const payload=folderAssetPayload(d,pid);
          if(!payload.public_url||!payload.s3_key) throw new Error(`Cannot normalize folder asset ${didAsset}: public_url=${Boolean(payload.public_url)} s3_key=${Boolean(payload.s3_key)}`);
  ...
  for(const s of devSongs){
    ...
    try{
      ...
      for(const d of da){const oldId=aid(d),url=aurl(d); if(!oldId||!url)continue; try{const existing=byUrl.get(url); if(existing){assetMap.set(oldId,aid(existing));result.direct_assets.reused++;continue} const payload=directPayload(sk,d); if(!payload.public_url||!payload.s3_key)throw new Error(`Cannot normalize direct asset ${oldId}`); const b=await req(`${PROD}/radio/admin/vec/song-assets`,{method:'POST',admin:prodToken,body:payload}); const p=b.asset||b.data?.asset||b.data||b; const newId=aid(p); if(!newId)throw new Error('create returned no asset id'); assetMap.set(oldId,newId); byUrl.set(url,p); result.direct_assets.created++}catch(e){result.direct_assets.failed.push({song_key:sk,asset_id:oldId,url,error:e.message})}}
